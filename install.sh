...
sudo apt-get -y install git python3 python3-pip hostapd dnsmasq libjpeg-dev zlib1g-dev libfreetype6-dev liblcms1-dev libopenjp2-7 libtiff5 libsecret-1-0 libsecret-1-dev libatlas-base-dev authbind python3-sdl2 autoconf dnsutils
...
# python packages
sudo pip3 install numpy pyserial timezonefinder pytz pydbus pygithub gpiozero Pillow flask pyowm pygame psutil waitress cython qrcode

# raspberry pi config
sudo raspi-config nonint do_spi 0 # enable SPI
sudo raspi-config nonint do_hostname "jjclock-$(xxd -l 2 -c 2 -p < /dev/random)" # set hostname
